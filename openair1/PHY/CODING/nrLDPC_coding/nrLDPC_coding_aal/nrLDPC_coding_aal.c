                            ...
                            int h, int i)
{
  ...
  bool special_case_tb_mode = !active_dev.is_t2 && (nrLDPC_slot_decoding_parameters->TBs[h].C == 1);
...
static int retrieve_ldpc_dec_op(struct rte_bbdev_dec_op **ops, nrLDPC_slot_decoding_parameters_t *nrLDPC_slot_decoding_parameters, int h, int i)
{
  int j = 0;
...
static int pmd_lcore_ldpc_dec(void *arg)
{
  ...
  set_ldpc_dec_op(ops_enq, bufs->inputs, bufs->hard_outputs, bufs->harq_outputs, nrLDPC_slot_decoding_parameters, h, r);

  // Start timer
  // We report timing only once in (0,0) since the timers are merged at the end
  start_meas(&nrLDPC_slot_decoding_parameters->TBs[h].segments[r].ts_ldpc_decode);
  ...
  // Stop timer
  // We report timing only once in (0,0) since the timers are merged at the end
  stop_meas(&nrLDPC_slot_decoding_parameters->TBs[h].segments[r].ts_ldpc_decode);

  if (deq == enq) {
    ret = retrieve_ldpc_dec_op(ops_deq, nrLDPC_slot_decoding_parameters, h, r);
...
int32_t nrLDPC_coding_decoder(nrLDPC_slot_decoding_parameters_t *nrLDPC_slot_decoding_parameters)
{
  for (int h = 0; h < nrLDPC_slot_decoding_parameters->nb_TBs; ++h) {
    for (int r = 0; r < nrLDPC_slot_decoding_parameters->TBs[h].C; r++) {
      ...
      ret = start_pmd_dec(&active_dev, op_params, &data_buffers, nrLDPC_slot_decoding_parameters, h, r);
      if (ret < 0) {

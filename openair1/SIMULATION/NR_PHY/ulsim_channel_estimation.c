...
/*
Holds params needed for PUSCH resource mapping
*/
typedef struct {
  rnti_t rnti;
  unsigned int K_ptrs;
...
void dmrs_ulsch(PHY_VARS_NR_UE *UE,
                ...
                c16_t **txdataF)
{
  ...
  uint32_t available_bits = nr_get_G(nb_rb, number_of_symbols, nb_dmrs_re_per_rb, number_dmrs_symbols, unav_res, mod_order, Nl);

  /////////////////////////ULSCH layer mapping/////////////////////////

  const int sz = available_bits / mod_order / Nl;
  c16_t ulsch_mod[Nl][sz];
  memset(ulsch_mod, 0, sizeof(ulsch_mod));

  // Do not do layer mapping since we only insert DMRS for stimulating channel estimation
  ...
  if (pusch_pdu->transform_precoding == transformPrecoder_enabled) {
    ...
    // Do not do DFT since we only insert DMRS symbols for stimulating channel estimation
...
void dmrs_nrUE_TX(PHY_VARS_NR_UE *ue, const UE_nr_rxtx_proc_t *proc, nr_phy_data_tx_t *phy_data)
{
  ...
  dmrs_ulsch(ue, frame_tx, slot_tx, phy_data, (c16_t **)&txdataF);
...
int channel_estimation_gNB_uespec_RX(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx, NR_UL_IND_t *UL_INFO)
{
    ...
    for (trial = 0; trial < n_trials; trial++) {
      ...
      while (round < max_rounds && crc_status) {
        ...
        if (input_fd == NULL) {
          ...
          dmrs_nrUE_TX(UE, &UE_proc, &phy_data);
        ...
        ul_proc_error = channel_estimation_gNB_uespec_RX(gNB, frame, slot, &UL_INFO);
